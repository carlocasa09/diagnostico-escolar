...
# --- CONFIGURACIÓN DE APIS Y CORREO ---
GROQ_API_KEY = "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" 
client = Groq(api_key=GROQ_API_KEY)
MODELO_IA = "llama-3.1-8b-instant"

MI_CORREO = "carlocasa09@gmail.com"  # Configurado según tu código anterior
MI_PASSWORD = "uoyt ykql kjcf pvtb" 
...
for key in keys:
    if key not in st.session_state:
        if key in ['paso_global']: st.session_state[key] = 1
...
if not st.session_state.empezar:
    st.markdown("""
        <div class="header-container">
            ...
            <p>Bienvenido al sistema de evaluación inteligente</p>
        </div>
    """, unsafe_allow_html=True)
    
    with st.container():
        st.info("👋 Por favor, ingresa tus datos para comenzar la evaluación.")
        with st.form("registro"):
            ...
            tema = st.text_input("Tema de la evaluación (Ej: Movimiento Circular):")
            ...
            if st.form_submit_button("Iniciar Evaluación 🚀"):
                if nom and tema:
                    ...
                    st.warning("⚠️ Completa todos los campos para continuar.")
...
# --- 6. PANTALLA DE EXAMEN ---
    ...
    if not st.session_state.lote_preguntas:
        with st.spinner("✨ La IA está preparando tus preguntas personalizadas..."):
    ...
    if st.session_state.lote_preguntas:
        # Barra de progreso
        progreso = st.session_state.paso_global / 10
...
# --- 7. PANTALLA FINAL ---
    ...
    st.markdown("""
        <div class="header-container">
            ...
            <p>Has completado la evaluación satisfactoriamente.</p>
        </div>
    """, unsafe_allow_html=True)
    
    st.write(f"### Gracias, {st.session_state.datos_alumno['nom']}.")
    st.write("Tus respuestas han sido enviadas al profesor. Puedes cerrar esta ventana.")
    
    if not st.session_state.correo_enviado:
        with st.spinner("📨 Generando informe pedagógico para el docente..."):
            df_reporte = pd.DataFrame(st.session_state.reporte)
            prompt_final = f"Analiza estos resultados: {st.session_state.reporte}. Alumno de {st.session_state.datos_alumno['gra']}. ¿Necesita adecuación?"
            ...
            exito = enviar_informe_docente(st.session_state.datos_alumno['nom'], analisis, df_reporte)
            if exito:
                st.session_state.correo_enviado = True
                st.toast("Informe enviado al correo del docente.")
